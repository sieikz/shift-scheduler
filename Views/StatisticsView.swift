...
struct StatisticsView: View {
            ...
            if monthlyEarningsData.isEmpty {
                ...
                .chartYAxis {
                    AxisMarks(position: .leading) { value in
                        AxisValueLabel {
                            if let earnings = value.as(Int.self) {
                                Text("¥\(earnings/1000)k")
                            }
                        }
                    }
                }
    ...
    private var monthlyEarningsData: [MonthlyEarning] {
        let calendar = Calendar.current
        var data: [MonthlyEarning] = []
        ...
        for i in 0..<6 {
            ...
            let monthShifts = shiftViewModel.shifts.filter { shift in
                calendar.isDate(shift.date, equalTo: monthDate, toGranularity: .month)
            }
            
            let monthStats = calculateStats(for: monthShifts)
            let monthFormatter = DateFormatter()
            monthFormatter.dateFormat = "M月"
            
            data.append(MonthlyEarning(
    ...
    private func calculateStats(for shifts: [Shift]) -> MonthlyStats {
        ...
        for shift in shifts {
            ...
            // 基本給与計算
            let baseEarnings = (workingMinutes * workplace.hourlyWage) / 60
            
            // 深夜手当
            let nightMinutes = shift.nightWorkingMinutes()
            let nightEarnings = Int(Double(nightMinutes * workplace.hourlyWage) * (workplace.nightShiftRate - 1.0) / 60)
            
            // 休日手当
            let holidayEarnings = shift.isHoliday() ? Int(Double(workingMinutes * workplace.hourlyWage) * (workplace.holidayRate - 1.0) / 60) : 0
            
            let shiftEarnings = baseEarnings + nightEarnings + holidayEarnings + workplace.transportationAllowance
