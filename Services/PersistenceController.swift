...
class PersistenceController: ObservableObject {
    ...
    func createRecurringShifts(_ shift: Shift) {
        ...
        while currentDate <= endDate {
            ...
            newShift.startTime = calendar.date(bySettingHour: startComponents.hour ?? 0, 
                                             minute: startComponents.minute ?? 0, 
                                             second: 0, of: currentDate) ?? currentDate
            newShift.endTime = calendar.date(bySettingHour: endComponents.hour ?? 0, 
    ...
    func fetchWorkplaces() {
        let request: NSFetchRequest<WorkplaceEntity> = WorkplaceEntity.fetchRequest()
        request.sortDescriptors = [NSSortDescriptor(keyPath: \WorkplaceEntity.priority, ascending: true)]
    ...
    func fetchShifts() {
        let request: NSFetchRequest<ShiftEntity> = ShiftEntity.fetchRequest()
        request.sortDescriptors = [NSSortDescriptor(keyPath: \ShiftEntity.date, ascending: true)]
